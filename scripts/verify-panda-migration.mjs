...
if (!existsSync(join(root, "styled-system", "styles.css")))
  fail("styled-system/styles.css missing; run `bun run panda:once`");

// 2) Ensure main.tsx imports Panda styles
try {
  const main = readFileSync(join(root, "src", "main.tsx"), "utf8");
  if (!main.includes("styled-system/styles.css"))
    fail("src/main.tsx is not importing styled-system/styles.css");
} catch (e) {
...
try {
  ...
  if (/tailwindcss|@tailwindcss\/postcss/i.test(postcss))
    fail("postcss.config.js still references Tailwind");
} catch (e) {
  // ok if missing
}
...
function grepDir(dir) {
  for (const name of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (name.isDirectory()) grepDir(p);
      ...
      if (
        /className=\"[^\"]*(?:\btext-|\bbg-|\bflex|\bgrid|\brounded|\bp-[xytrbl]?|\bm-[xytrbl]?|\bw-\d)/.test(
          s,
        )
      ) {
